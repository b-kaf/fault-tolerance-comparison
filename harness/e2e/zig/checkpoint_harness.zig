...
// Working checkpointed record (active + saved copy). Held in a single
// contiguous struct so it is memory-resident at the injection point and
// corruptible exactly like the C harness's `harness_checkpoint_state`. The
// algorithm runs in place on it; mirrorState only publishes the two observable
// values the tooling reads.
export var harness_checkpoint_state: checkpoint.CheckpointedRecord = undefined;
...
fn mirrorState() void {
    store(&harness_last_active_value, harness_checkpoint_state.active.value);
    store(&harness_last_checkpoint_value, harness_checkpoint_state.checkpoint.value);
}
...
export fn harness_main() callconv(.c) noreturn {
    ...
    while (true) {
        ...
        harness_checkpoint_state = checkpoint.CheckpointedRecord.init(sampleRecord(initial));

        store(&harness_stage, abi.stage.after_checkpoint);
        _ = harness_checkpoint_state.capture();

        harness_checkpoint_state.active.value = expected;
        harness_checkpoint_state.active.refreshChecksum();
        mirrorState();

        store(&harness_stage, abi.stage.after_mutation);
        @call(.never_inline, harness_injection_point_after_mutation, .{});

        applyPendingFault(&harness_checkpoint_state);

        store(&harness_stage, abi.stage.before_commit);
        const result = harness_checkpoint_state.commitOrRestart();
