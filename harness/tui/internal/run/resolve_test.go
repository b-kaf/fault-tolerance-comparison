...
func TestResolveE2ERejectsHugeIterations(t *testing.T) {
	_, err := ResolveE2E("/does-not-exist", config.DefaultSettings(), "tmr", "zig", "m4", "mixed", maxRunCount+1, "")
...
func TestResolveFuzzRejectsHugeTrials(t *testing.T) {
	_, err := ResolveFuzz("/does-not-exist", config.DefaultSettings(), "tmr", "zig", "m4", "reg-bitflip", maxRunCount+1, "0x1", "")
